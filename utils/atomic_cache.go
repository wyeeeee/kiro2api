...
func (atc *AtomicTokenCache) GetHot() (*types.TokenInfo, bool) {
	...
	token, ok := value.(*types.TokenInfo)
	if !ok || token == nil {
		atc.misses.Add(1)
		return nil, false
	}
	...
	if token.IsExpired() {
		// 原子地清除过期的热点token
		atc.hot.CompareAndSwap(value, nil)
		atc.misses.Add(1)
		return nil, false
	}
...
func (atc *AtomicTokenCache) Delete(idx int) {
	...
	if atc.hotIdx.Load() == int64(idx) {
		atc.hot.Store((*types.TokenInfo)(nil))
		atc.hotIdx.Store(0)
	}
}
...
func (atc *AtomicTokenCache) GetStats() map[string]any {
	...
	return map[string]any{
		...
		"hit_rate": hitRate,
		"size":     size,
		"hot_idx":  atc.hotIdx.Load(),
		"has_hot":  atc.hot.Load() != nil,
	}
}
