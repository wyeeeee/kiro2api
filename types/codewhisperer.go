...
func NewAssistantResponseEventFromJSON(jsonStr string) (*AssistantResponseEvent, error) {
	event := &AssistantResponseEvent{}
	if err := event.FromJSON(jsonStr); err != nil {
		return nil, err
	}
	return event, nil
}

// Clone 深度复制AssistantResponseEvent
func (are *AssistantResponseEvent) Clone() (*AssistantResponseEvent, error) {
	jsonStr, err := are.ToJSON()
	if err != nil {
		return nil, fmt.Errorf("克隆失败(序列化): %w", err)
	}

	return NewAssistantResponseEventFromJSON(jsonStr)
}
